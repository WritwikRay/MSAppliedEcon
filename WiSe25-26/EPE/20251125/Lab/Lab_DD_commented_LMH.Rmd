...
output: pdf_document
---
DON'T FORGET TO SHOW THE STUDENTS THIS LINK (FROM THE LITERATURE IN THE SYLLABUS)!!!!!!!!!!!!!!
https://theeffectbook.net/ch-DifferenceinDifference.html?panelset=r-code3
...
# Use pacman to install and load all packages
pacman::p_load(
  AER,
  clubSandwich,
  compositions,
...
dd.df <- subset(evaluation.df, treatment_locality == 1) # the household does get the treatment
dd.df$eligible_round <- dd.df$eligible*dd.df$round #the household are eligible and we take into account of the treatment effect after the treatment
...
ex8_lm1 <- lm(health_expenditures ~ eligible_round + round + eligible, 
             data = dd.df)
ex8_lm1.5 <- lm(health_expenditures ~ round, 
             data = dd.df)
# lm + interaction
...
models8 <- list("ex8_lm1" = ex8_lm1, "ex8_lm1.5" = ex8_lm1.5, "ex8_lm2" = ex8_lm2, "ex8_feols1" = ex8_feols1)
...
models9 <- list("ex8_feols1" = ex8_feols1, "ex9_lm1" = ex9_lm1, "ex9_feols1" = ex9_feols1)
modelsummary(list(ex8_feols1,ex9_lm1,ex9_feols1), 
             vcov = ~ locality_identifier, 
             stars = c("*" = .1, "**" = .05, "***" = .01), 
             fmt = 3, 
             gof_omit = "AIC|BIC|Log.Lik.|R2 Adj.|R2 Within|R2 Pseudo|F")
...
# QUIZ QUESTIONS
1.	Explain in words what DD does
DD is a quasi-experiment that takes the natural abruption as the randomization to understand the sharp change in economics.

2.	Explain the importance of the common trends assumption:
What is common trends assumption 
i.	We assume that the treatment group and the control group have the common trajectory before the abruption events happens.
ii.	It is important because the common trends assumption assure that the change in the outcome of the experiment is not due to other variables than the treatment in this case.

3.	Which type of data that we should not use to implement 
Normally I think it would be non-linear data

4.	Example of a placebo test – “fake” outcome, using fake treatment group, for example a group of population you know that is not being affected by the treatment. 
