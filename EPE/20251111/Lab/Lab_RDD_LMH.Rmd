...
#we are recentering the income distribution, if smaller of equal to 58, recenter it.
...
comment:The non-linear function around the threshold in RDD refers to the way the relationship between treatment assignment and outcome variable behaves near the cutoff point. This can involve employing polynomial functions, spline functions, or other flexible models to capture potential non-linearities in the relationship between the treatment and outcomes as they approach and move away from the threshold.
...
# Add the second set of points with a different color
points(rd.df$poverty_index, rd.df$poverty_index_right, col = 2)

# Add a legend to the plot
legend("topright", # Position of the legend
       legend = c("Left Transformation", "Right Transformation"), # Labels
       col = c(1, 2), # Colors matching the points
       pch = c(1, 1)) # Point symbols
...
# Quadratic Component Plot
plot(rd.df$poverty_index, rd.df$poverty_index_left2, 
     col = 3, xlim = c(0, 100), ylim = c(-100, 100), 
     xlab = "Poverty Index", ylab = "Transformation", 
     main = "Quadratic Component")
...
# Add a legend
legend("topright", 
       legend = c("Left Quadratic Transformation", "Right Quadratic Transformation"), 
       col = c(3, 4), 
       pch = c(1, 1))
...
# Cubic Component Plot
plot(rd.df$poverty_index, rd.df$poverty_index_left3, 
     col = 5, xlim = c(0, 100), ylim = c(-100, 100), 
     xlab = "Poverty Index", ylab = "Transformation", 
     main = "Cubic Component")
...
# Add a legend
legend("topright", 
       legend = c("Left Cubic Transformation", "Right Cubic Transformation"), 
       col = c(5, 6), 
       pch = c(1, 1))
...
# Combined Plot for Terms of Cubic Polynomial
plot(rd.df$poverty_index, rd.df$poverty_index_left, 
     xlim = c(0, 100), ylim = c(-100, 100),  
     xlab = "Poverty Index", ylab = "Transformation", 
     main = "Terms of Cubic Polynomial")

# Adding points for each transformation
points(rd.df$poverty_index, rd.df$poverty_index_right, col = 2)   # Right linear
points(rd.df$poverty_index, rd.df$poverty_index_left2, col = 3)  # Left quadratic
points(rd.df$poverty_index, rd.df$poverty_index_right2, col = 4) # Right quadratic
points(rd.df$poverty_index, rd.df$poverty_index_left3, col = 5)  # Left cubic
points(rd.df$poverty_index, rd.df$poverty_index_right3, col = 6) # Right cubic

# Adding a legend to distinguish each component
legend("topright", 
       legend = c("Left Linear", "Right Linear", "Left Quadratic", 
                  "Right Quadratic", "Left Cubic", "Right Cubic"), 
       col = c(1, 2, 3, 4, 5, 6), 
       pch = 1)
...
# THE IDEA BEHIND QUICKLY EXPLAINED BY THE PROFESSOR: 
# in this distribution of poverty index, maybe the super rich and the super poor are not effect by the treatment much. We should concentrate on around the threshold. 
...
ex_lm_full <- lm(health_expenditures ~ eligible + 
                 poverty_index_left + poverty_index_right, 
                     data = rd.df) #full sample
# Window +-40: starting to slide the study
...
.#when you have less observation, then you have bigger S.E
...
# QUiZ QUESTIONS
1.	RD provides an estimate of LATE instead of ATE
a.	Because it is the estimation of difference of the non-treated and treated at the threshold (less power than asserting the whole sample)

2.	The difference between sharp and fuzzy discontinuity:
a.	Sharp continuity is when the discontinuity precisely determines the treatment. In other words, i.e. those who are above the threshold receive the treatment and those who are under it don’t receive the treatment. 
b.	Fuzzy discontinuity is when the difference for the individual above and under the threshold does not totally determine if these 2 sub-groups receive the treatment or not. For example, sometimes this can be due to marginal differences.

3.	Why RD would suffer more from power problem: because the estimation is being done around the threshold. Hence, this would reduce the number of observation than the total sample size

4.	How one should implement RD, which regression one would estimate
a.	We would estimate the participation of the program along with constructing an index on around the threshold (i.e. if the individual has the income higher than a threshold –> assign value 1, if not assign value 0). Then we estimate the effect it has one the outcome of the study. Difference of the CG and TG at post-treatment data

5.	Which type of data that RD would be implemented: continuous data for the eligibility index
